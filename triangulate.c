...
int update_boundary_triangulation(arr3 v1, arr3 v2, arr3 v3, ptriangulation result) {
  ...
  {
    rem_boundary_triangulation(idx, result);
    return -2;
  }
  printf("New boundary for triangulation:\n");
  print_triangle(triang);
  result->bound_len++;
  result->bound_tri = realloc(result->bound_tri,result->bound_len * sizeof(triangle));
  result->bound_tri[result->bound_len - 1] = *tri;
}
...
triangulation triangulate_cube(data_list * data) { 
