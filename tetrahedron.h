...
struct Tetrahedron  : public Simplex<3,4> 
{
  using Simplex<3,4>::_vertices;
  // move constructor
  inline Tetrahedron(Simplex<3,4> &&other)  : Simplex<3,4>(move(other)) {}
...
inline bool Tetrahedron::acute(const Tetrahedron &tet)
{
  Vector<3> edges[5];
  Vector<3> normals[4];
...
inline bool Tetrahedron::acute(const Triangle<3> &triangle, const Vector<3> &pt)
{
  Vector<3> edges[5];
  Vector<3> normals[4];
