...
class Domain {
  ...
  public:
    ...
    inline bool boundary(const Triangle<_dim> &triang) const { return super()->boundary(triang); }
};
...
class Cube : public Domain<3, Cube> {
  public:
    ...
    // static to return point from cube idx
    //inline static Vector<3> vertex(vindex idx, byte scale) const {

    inline bool boundary(const Triangle<3> &triang) const {
