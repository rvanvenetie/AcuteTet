...
__global__ void tet_acute_kernel(triangle triang, int dim, unsigned char * result, size_t N) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx>= N)
    return;
  arr3 P[5]; 
...
int main(void)
{
  ...
  tet_acute_kernel <<< cube_pts.len/THREADS_BLOCK + 1  , THREADS_BLOCK >>> (triang, DIM, res_d, cube_pts.len);
