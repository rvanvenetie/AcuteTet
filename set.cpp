...
/**
 *  TSparseSet implementation
 */
// initalization method
tempD void TSparseSet<D>::init(tindex axis, byte scale, bool set) {
  _name = "TSparseSet";
  _scale = scale;
  _size = axis;
...
// from file
tempD void TSparseSet<D>::init(ifstream &file, tindex axis, byte scale) 
{
  init(axis, scale, false);
  if (!file.good()) throw std::runtime_error("Failed to load from file");
  for (size_t i = 0; i < size(); i++) {
...
// returns whether the axis is empty
tempD bool TSparseSet<D>::empty(vindex i, vindex j) const
{
...
// to file
tempD bool TSparseSet<D>::toFile(const std::string &filename, bool sparse) const 
{
  // only save in sparse mode.. C'mon!
...
template class TSparseSet<3>;
template class TSet<TSparseSet<3>,3>;
